...
public class NotificationProcessor : INotificationProcessor
{
    ...
    private static NotificationContent BuildNotificationContent(NotificationTemplate template, Dictionary<string, object> variables)
    {
        var content = new NotificationContent
        {
            Subject = ReplaceVariables(template.Subject, variables),
            Body = ReplaceVariables(template.Body, variables) ?? string.Empty,
            Variables = variables.ToDictionary(kv => kv.Key, kv => kv.Value?.ToString() ?? string.Empty)
        };
